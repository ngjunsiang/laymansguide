...
You can, of course, change this setting (and I won’t teach you how to do that here, though it is just a Google search away). If you disable Recycle Bin, or use the `Shift+Delete` hotkey combi to force a permanent deletion, something different happens instead. Intuitively, it seems like the OS should actually clear out the (digital) space previously occupied by the file contents. But this offers no advantage; to change a `1` to a `0`, you have to *write* a 0 to it; there is no *erasing* in the world of storage!
